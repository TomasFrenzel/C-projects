...
void Nasobilka(int n) {
    ...
    // Výpis násobilky až do "n x n"
    for (i = 1; i <= n; i++) {  
        for (j = 1; j <= n; j++) {
            printf("%d * %d = %d\n", i, j, i * j);  // Výpis násobení ve formátu "i * j = výsledek"
        }
    }
}
...
float AritmetickyPrumer(int pole[], int velikost) {
    int soucet = 0;

    // Procházení pole a sčítání hodnot
    for (int i = 0; i < velikost; i++) {
        soucet += pole[i];
    }

    // Vypočítání průměru
    return (float)soucet / velikost;
}
